...
const (
	...
	// Websocket client.
	WS
	// HTTP Client (GraphQL API & GraphQL Playground)
	GRAPH_API
)
	...
	for {
		...
		for i := 0; i < len(bufs); i++ {
			if err := c.parse(bufs[i]); err != nil {
				...
				//if _, err := bufio.NewReader(conn).WriteTo(c.nc); err == nil {
				//	isGraphAPI = true
				//}else {
				//	fmt.Print(err, " Err Writing bytes boi")
				//}

				//fmt.Print(string(bufs[i]), "Buff I: \n")

...
func (s *Server) GetConnectionListener() net.Listener {
	return s.listener
}
...
func (s *Server) acceptConnections(l net.Listener, acceptName string, createFunc func(conn net.Conn), errFunc func(err error) bool) {
	...
	for {
		...
		//rootGraphConn, err := net.Dial("tcp", "127.0.0.1:9000")
		//if err != nil {
		//	log.Fatal(err)
		//}
		//
		//go func() {
		//	_, _ = io.Copy(rootGraphConn, conn)
		//}()
		//conBackup := context.WithValue(context.TODO(), "conn", conn)
		...
		//
		//go func() {
		//	_, err = bytes.NewReader(b).WriteTo(rootGraphConn)
		//	if err != nil {
		//		s.isGraphConn <- conz.NatsCon
		//		log.Fatal(err)
		//	}
		//}()

		//fmt.Fprintf(rootGraphConn, string(b))

		//buf, err := ioutil.ReadAll(conn)
		//if err != nil {
		//	log.Fatal(err)
		//}
		//fmt.Print(string(buf), " Read Buff \n")

		//conn = conBackup.Value("conn").(net.Conn)
		//select {
		//case c := <-s.isGraphConn:
		//	switch c {
		//	case conz.GraphCon:
		//		continue
		//	//case conz.Unknown:
		//	//	fmt.Fprint(conn, errors.New("invalid request"))
		//	//	conn.Close()
		//	default:
		//		goto skipState
		//	}
